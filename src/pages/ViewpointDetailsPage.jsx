...
const ViewpointDetailsPage = () => {
  ...
  useEffect(() => {
    const unsubscribe = onTourismSitesByCategoryChange('viewpoints', (sites) => {
  ...
  if (!viewpoint) return <Box sx={{ color: '#fff', textAlign: 'center', mt: 8 }}>لم يتم العثور على نقطة المشاهدة هذه.</Box>;
        ...
        {viewpoint.images && viewpoint.images.length > 0 && (
          <>
            ...
            {viewpoint.images.length > 1 && (
              <>
                <IconButton
                  onClick={() => setCurrentImage((currentImage - 1 + viewpoint.images.length) % viewpoint.images.length)}
                  sx={{ 
                    position: 'absolute', 
                    top: '50%', 
                    left: { xs: 5, sm: 10 }, 
                ...
                <IconButton
                  onClick={() => setCurrentImage((currentImage + 1) % viewpoint.images.length)}
                  sx={{ 
                    position: 'absolute', 
                    top: '50%', 
                    right: { xs: 5, sm: 10 }, 
