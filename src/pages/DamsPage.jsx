...
const DamsPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = onTourismSitesByCategoryChange('dams', (sites, error) => {
      if (error) {
  ...
  const openInGoogleMaps = async (dam) => {
    try {
      ...
      if (dam.googleMapsUrl) {
        ...
        url = `https://www.google.com/maps/search/${encodeURIComponent(dam.title + ' ' + dam.address)}`;
      }
      ...
      <Container sx={{ py: 8, position: 'relative', zIndex: 1 }}>
                ...
                >
                  ...
                  <Box 
                    sx={{ 
                      ...
                      alignItems: 'center',
                      justifyContent: 'center',
                      cursor: dam.images && dam.images.length > 1 ? 'grab' : 'default',
                      '&:active': {
                        cursor: dam.images && dam.images.length > 1 ? 'grabbing' : 'default'
                      }
                    }}
                    onTouchStart={dam.images && dam.images.length > 1 ? handleTouchStart : undefined}
                    onTouchMove={dam.images && dam.images.length > 1 ? handleTouchMove : undefined}
                    onTouchEnd={dam.images && dam.images.length > 1 ? () => handleTouchEnd(dam.id, dam.images.length) : undefined}
                    onMouseDown={dam.images && dam.images.length > 1 ? handleMouseDown : undefined}
                    onMouseMove={dam.images && dam.images.length > 1 ? handleMouseMove : undefined}
                    onMouseUp={dam.images && dam.images.length > 1 ? () => handleMouseUp(dam.id, dam.images.length) : undefined}
                    onMouseLeave={dam.images && dam.images.length > 1 ? () => handleMouseUp(dam.id, dam.images.length) : undefined}
                  >
                        ...
                        {dam.images.map((_, index) => (
                          <Box
                            ...
                            sx={{
                              ...
                              height: 8,
                              borderRadius: '50%',
                              background: (currentImageIndex[dam.id] || 0) === index 
        ...
        {damsData.length === 0 && !loading && (
          <Box sx={{ textAlign: "center", py: 8 }}>
            <Water sx={{ fontSize: 80, color: "rgba(255,255,255,0.3)", mb: 2 }} />
            <Typography
              variant="h4"
              sx={{
                color: "#fff",
