...
const ParksPage = () => {
    ...
    const unsubscribe = onTourismSitesByCategoryChange(
      ...
      (sites, error) => {
        if (error) {
          ...
          console.log("Number of parks:", sites.length);
        ...
        const updatedParks = await Promise.all(
          parksData.map(async (park) => {
            ...
            try {
              ...
              console.warn(`Failed to fetch Google info for ${park.title}:`, error);
            }
  ...
  const addSampleParks = async () => {
    try {
      const sampleParks = [
        {
          title: "منتزه الأمير محمد بن سعود",
          ...
          address: "حي النخيل، بني حسن، الباحة",
          googleMapsUrl: "https://maps.google.com/?q=بني+حسن+الباحة",
          openingHours: "6:00 صباحاً - 10:00 مساء",
          contactInfo: "0172345678",
          price: "10 ريال للكبار، 5 ريال للأطفال",
          rating: 4.4,
          reviewsCount: 89,
          coordinates: { lat: 20.0129, lon: 41.4677 },
          activities: ["ألعاب أطفال", "مساحات خضراء", "مواقف سيارات", "دورات مياه"],
          isActive: true,
          category: "parks"
        },
        {
          ...
          address: "طريق غابة رغدان، بني حسن، الباحة",
          googleMapsUrl: "https://maps.google.com/?q=غابة+رغدان+الباحة",
          openingHours: "5:00 صباحاً - 8:00 مساء",
          contactInfo: "هيئة تطوير المنطقة 0501876543",
          price: "مجاني",
          rating: 4.8,
          reviewsCount: 134,
          coordinates: { lat: 20.0234, lon: 41.4567 },
          activities: ["مسارات مشي", "تخييم", "مراقبة الطيور", "هواء نقي"],
          isActive: true,
          category: "parks"
        }
      ];
      ...
      console.log("تم إضافة المنتزهات التجريبية بنجاح");
    } catch (error) {
      console.error("خطأ في إضافة المنتزهات التجريبية:", error);
    }
  };
      ...
      <Container sx={{ py: 8, position: "relative", zIndex: 1 }}>
        ...
        >
          {parksData.map((park, idx) => {
            console.log(`Rendering park ${idx + 1}:`, {
              title: park.title,
              hasImages: park.images && park.images.length > 0,
                  ...
                  >
                    ...
                    >
                      ...
                      {/* {park.weather && (
                        ...
                        >
                          <img
                            src={`https://openweathermap.org/img/wn/${park.weather.icon}.png`}
                            alt="weather"
                            style={{
                              width: "24px",
                              height: "24px",
