...
const FarmDetailsPage = () => {
  ...
  useEffect(() => {
    const unsubscribe = onTourismSitesByCategoryChange('farms', (sites) => {
      const found = sites.find(f => f.id === id);
      setFarm(found || null);
      setLoading(false);
  ...
  if (!farm) return <Box sx={{ color: '#fff', textAlign: 'center', mt: 8 }}>لم يتم العثور على هذه المزرعة.</Box>;
        ...
        {farm.images && farm.images.length > 0 && (
          <>
            ...
            {farm.images.length > 1 && (
              <>
                <IconButton
                  onClick={() => setCurrentImage((currentImage - 1 + farm.images.length) % farm.images.length)}
                  sx={{ 
                    position: 'absolute', 
                    top: '50%', 
                    left: { xs: 5, sm: 10 }, 
                ...
                <IconButton
                  onClick={() => setCurrentImage((currentImage + 1) % farm.images.length)}
                  sx={{ 
                    position: 'absolute', 
                    top: '50%', 
                    right: { xs: 5, sm: 10 }, 
        ...
        <Box sx={{ 
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Typography sx={{ fontSize: 40, fontWeight: 'bold', color: 'white', lineHeight: 1 }}>
                {weather?.temp ? `${weather.temp}Â°` : '--Â°'}
              </Typography>
              <WbSunnyIcon sx={{ fontSize: 32, color: 'white' }} />
            </Box>
            
            <Box sx={{ width: '2px', height: 60, bgcolor: 'white', mx: 2, borderRadius: 1 }} />
