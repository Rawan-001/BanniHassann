...
import { Thermostat, Star, Place, Home } from "@mui/icons-material";
...
const HousingDetailsPage = () => {
  ...
  useEffect(() => {
    const unsubscribe = onTourismSitesByCategoryChange('housing', (sites) => {
      const found = sites.find(h => h.id === id);
      setHousing(found || null);
      setLoading(false);
  ...
  if (!housing) return <Box sx={{ color: '#fff', textAlign: 'center', mt: 8 }}>لم يتم العثور على هذا النزل.</Box>;

  return (
    <Box sx={{ background: '#000', minHeight: '100vh', py: 0, position: 'relative' }}>
      {/* زر الرجوع أعلى يمين الصفحة */}
      <Box sx={{ 
        position: 'fixed', 
        ...
        {housing.images && housing.images.length > 0 && (
          <>
            ...
            {housing.images.length > 1 && (
              <>
                <IconButton
                  onClick={() => setCurrentImage((currentImage - 1 + housing.images.length) % housing.images.length)}
                  sx={{ 
                    position: 'absolute', 
                    top: '50%', 
                    left: { xs: 5, sm: 10 }, 
                ...
                <IconButton
                  onClick={() => setCurrentImage((currentImage + 1) % housing.images.length)}
                  sx={{ 
                    position: 'absolute', 
                    top: '50%', 
                    right: { xs: 5, sm: 10 }, 
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Star sx={{ fontSize: 32, color: 'white', stroke: '#fff', strokeWidth: 1, fill: 'none' }} />
              <Typography sx={{ fontSize: 32, fontWeight: 'bold', color: 'white', lineHeight: 1 }}>
                {housing.rating ? housing.rating : '--'}
              </Typography>
            </Box>
          </Box>
          {/* زر الوصول للموقع */}
          <Button
            ...
            onClick={() => window.open(housing.googleMapsUrl, '_blank')}
            startIcon={<Place sx={{ 
              color: 'white', 
        ...
        {/* العمود الأيمن: النبذة والوصف والفعاليات */}
        <Box sx={{ 
          flex: 1, 
          minWidth: 0, 
          order: { xs: 1, md: 2 }, 
          ...
          {/* الفعاليات المصاحبة - تظهر فقط للسكن الذي لديه تفاصيل إضافية */}
          {housing.details && housing.details.length > 0 && (
            <Box sx={{ mt: { xs: 3, sm: 3.5, md: 4 } }}>
              ...
              <Box sx={{ 
                ...
                {housing.details.map((detail, idx) => {
                  // خريطة الأيقونات
                  const iconsMap = {
                  ...
                  const IconComp = iconsMap[detail.icon] || LocalHotelIcon;
                  return (
          ...
          {housing.activities && housing.activities.length > 0 && (
            <Box sx={{ mt: { xs: 3, sm: 3.5, md: 4 } }}>
              <Typography 
                variant="h4" 
                sx={{ 
              ...
              <Box sx={{ 
                ...
                {housing.activities.map((act, idx) => (
                  <Box key={idx} sx={{ 
                    display: 'flex', 
