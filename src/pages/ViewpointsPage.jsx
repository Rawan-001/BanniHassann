...
const ViewpointsPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = onTourismSitesByCategoryChange('viewpoints', (sites, error) => {
      if (error) {
        console.error("حدث خطأ في تحميل البيانات:", error);
        setError("حدث خطأ في تحميل البيانات");
        setViewpointsData([]);
      } else {
        console.log("Viewpoints data received:", sites);
        sites.forEach((viewpoint, index) => {
          console.log(`Viewpoint ${index + 1} (${viewpoint.title}):`, {
            id: viewpoint.id,
            images: viewpoint.images,
            hasImages: viewpoint.images && viewpoint.images.length > 0
          });
        });
  ...
  const openInGoogleMaps = async (viewpoint) => {
    try {
      ...
      if (viewpoint.googleMapsUrl) {
        ...
        url = `https://www.google.com/maps/search/${encodeURIComponent(viewpoint.title + ' ' + viewpoint.address)}`;
      }
      ...
      <Container sx={{ py: 8, position: 'relative', zIndex: 1 }}>
        ...
        >
          {viewpointsData.map((viewpoint, idx) => {
            console.log(`Rendering viewpoint ${idx + 1}:`, {
              title: viewpoint.title,
              hasImages: viewpoint.images && viewpoint.images.length > 0,
              imagesCount: viewpoint.images ? viewpoint.images.length : 0,
              firstImage: viewpoint.images && viewpoint.images.length > 0 ? viewpoint.images[0] : null
            });
                ...
                >
                  <Box 
                    sx={{ 
                      ...
                      alignItems: 'center',
                      justifyContent: 'center',
                      cursor: viewpoint.images && viewpoint.images.length > 1 ? 'grab' : 'default',
                      '&:active': {
                        cursor: viewpoint.images && viewpoint.images.length > 1 ? 'grabbing' : 'default'
                      }
                    }}
                    onTouchStart={viewpoint.images && viewpoint.images.length > 1 ? handleTouchStart : undefined}
                    onTouchMove={viewpoint.images && viewpoint.images.length > 1 ? handleTouchMove : undefined}
                    onTouchEnd={viewpoint.images && viewpoint.images.length > 1 ? () => handleTouchEnd(viewpoint.id, viewpoint.images.length) : undefined}
                    onMouseDown={viewpoint.images && viewpoint.images.length > 1 ? handleMouseDown : undefined}
                    onMouseMove={viewpoint.images && viewpoint.images.length > 1 ? handleMouseMove : undefined}
                    onMouseUp={viewpoint.images && viewpoint.images.length > 1 ? () => handleMouseUp(viewpoint.id, viewpoint.images.length) : undefined}
                    onMouseLeave={viewpoint.images && viewpoint.images.length > 1 ? () => handleMouseUp(viewpoint.id, viewpoint.images.length) : undefined}
                  >
                    <Box 
                      sx={{ 
                        position: 'absolute',
                    ...
                    >
                      ...
                      {weather[viewpoint.title] && (
                        <Typography sx={{ fontSize: 14, fontWeight: 'bold', color: 'white' }}>
                          {weather[viewpoint.title].temp}Â°
                        </Typography>
                      )}
                    </Box>

                    {viewpoint.images && viewpoint.images.length > 0 && (() => {
                      const currentIndex = currentImageIndex[viewpoint.id] || 0;
                        ...
                        {viewpoint.images.map((_, index) => (
                          <Box
                            ...
                            sx={{
                              ...
                              height: 8,
                              borderRadius: '50%',
                              background: (currentImageIndex[viewpoint.id] || 0) === index 
        ...
        {viewpointsData.length === 0 && !loading && (
          <Box sx={{ textAlign: "center", py: 8 }}>
            <Visibility sx={{ fontSize: 80, color: "rgba(255,255,255,0.3)", mb: 2 }} />
            <Typography
              variant="h4"
              sx={{
                color: "#fff",
