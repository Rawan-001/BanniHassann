...
const CafesPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = onTourismSitesByCategoryChange('cafes', (sites, error) => {
      if (error) {
        console.error("Failed to fetch cafes in real-time:", error);
  ...
  const openInGoogleMaps = async (cafe) => {
    try {
      ...
      } else if (cafe.coordinates?.lat && cafe.coordinates?.lon) {
        url = `https://www.google.com/maps/@${cafe.coordinates.lat},${cafe.coordinates.lon},15z`;
      } else {
        url = `https://www.google.com/maps/search/${encodeURIComponent(cafe.title + ' ' + cafe.address)}`;
      }
      ...
      <Container sx={{ py: 8, position: 'relative', zIndex: 1 }}>
                ...
                >
                  ...
                  <Box 
                    sx={{ 
                      ...
                      alignItems: 'center',
                      justifyContent: 'center',
                      cursor: cafe.images && cafe.images.length > 1 ? 'grab' : 'default',
                      '&:active': {
                        cursor: cafe.images && cafe.images.length > 1 ? 'grabbing' : 'default'
                      }
                    }}
                    onTouchStart={cafe.images && cafe.images.length > 1 ? handleTouchStart : undefined}
                    onTouchMove={cafe.images && cafe.images.length > 1 ? handleTouchMove : undefined}
                    onTouchEnd={cafe.images && cafe.images.length > 1 ? () => handleTouchEnd(cafe.id, cafe.images.length) : undefined}
                    onMouseDown={cafe.images && cafe.images.length > 1 ? handleMouseDown : undefined}
                    onMouseMove={cafe.images && cafe.images.length > 1 ? handleMouseMove : undefined}
                    onMouseUp={cafe.images && cafe.images.length > 1 ? () => handleMouseUp(cafe.id, cafe.images.length) : undefined}
                    onMouseLeave={cafe.images && cafe.images.length > 1 ? () => handleMouseUp(cafe.id, cafe.images.length) : undefined}
                  >
                        ...
                        {cafe.images.map((_, index) => (
                          <Box
                            ...
                            sx={{
                              ...
                              height: 8,
                              borderRadius: '50%',
                              background: (currentImageIndex[cafe.id] || 0) === index 
        ...
        {cafesData.length === 0 && !loading && (
          <Box sx={{ textAlign: "center", py: 8 }}>
            <LocalCafe sx={{ fontSize: 80, color: "rgba(255,255,255,0.3)", mb: 2 }} />
            <Typography
              variant="h4"
              sx={{
                color: "#fff",
            ...
            >
              كن أول من يضيف مقهى جميل في بني حسن
            </Typography>
          </Box>
        )}
      </Container>
