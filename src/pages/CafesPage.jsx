...
import { ArrowBack, ArrowForward, LocalCafe, Star } from "@mui/icons-material";
...
const CafesPage = () => {
  ...
  const handleTouchEnd = useCallback((siteId, totalImages) => {
    ...
    if (isLeftSwipe) {
      handleNext(siteId, totalImages);
    }
    if (isRightSwipe) {
      handlePrev(siteId, totalImages);
    }
  ...
  const handleMouseUp = useCallback((siteId, totalImages) => {
    if (!touchStart || !touchEnd) return;
    ...
    const unsubscribe = onTourismSitesByCategoryChange(
      ...
      (sites, error) => {
        if (error) {
          ...
          setError("لا توجد مقاهي متاحة حالياً");
          setCafesData([]);
        } else {
          console.log("Cafes data received:", sites);
          console.log("Number of cafes:", sites.length);
  ...
  if (loading) {
    return <FastLoadingSpinner message="جاري تحميل المقاهي..." />;
  }
                          ...
                          {cafe.images.map((_, index) => (
                            <Box
                              ...
                              sx={{
                                ...
                                borderRadius: "50%",
                                background:
                                  (currentImageIndex[cafe.id] || 0) === index
