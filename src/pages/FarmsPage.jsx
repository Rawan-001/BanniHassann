...
const FarmsPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = onTourismSitesByCategoryChange('farms', (sites, error) => {
      if (error) {
  ...
  const openInGoogleMaps = async (farm) => {
    try {
      ...
      if (farm.googleMapsUrl) {
        ...
        url = `https://www.google.com/maps/search/${encodeURIComponent(farm.title + ' ' + farm.address)}`;
      }
      ...
      <Container sx={{ py: 8, position: 'relative', zIndex: 1 }}>
                ...
                >
                  ...
                  <Box 
                    sx={{ 
                      ...
                      alignItems: 'center',
                      justifyContent: 'center',
                      cursor: farm.images && farm.images.length > 1 ? 'grab' : 'default',
                      '&:active': {
                        cursor: farm.images && farm.images.length > 1 ? 'grabbing' : 'default'
                      }
                    }}
                    onTouchStart={farm.images && farm.images.length > 1 ? handleTouchStart : undefined}
                    onTouchMove={farm.images && farm.images.length > 1 ? handleTouchMove : undefined}
                    onTouchEnd={farm.images && farm.images.length > 1 ? () => handleTouchEnd(farm.id, farm.images.length) : undefined}
                    onMouseDown={farm.images && farm.images.length > 1 ? handleMouseDown : undefined}
                    onMouseMove={farm.images && farm.images.length > 1 ? handleMouseMove : undefined}
                    onMouseUp={farm.images && farm.images.length > 1 ? () => handleMouseUp(farm.id, farm.images.length) : undefined}
                    onMouseLeave={farm.images && farm.images.length > 1 ? () => handleMouseUp(farm.id, farm.images.length) : undefined}
                  >
                        ...
                        {farm.images.map((_, index) => (
                          <Box
                            ...
                            sx={{
                              ...
                              height: 8,
                              borderRadius: '50%',
                              background: (currentImageIndex[farm.id] || 0) === index 
        ...
        {farmsData.length === 0 && !loading && (
          <Box sx={{ textAlign: "center", py: 8 }}>
            <Agriculture sx={{ fontSize: 80, color: "rgba(255,255,255,0.3)", mb: 2 }} />
            <Typography
              variant="h4"
              sx={{
                color: "#fff",
